...
\begin{align}
	...
	\implies {\mathcal {Z}}\{x(n-1)\} &= z^{-1} + 2z^{-2} + 3z^{-3} + 4z^{-4} + 2z^{-5} + z^{-6} \\
									  &= z^{-1}X(z)
\end{align}	
\item Find
\begin{equation}
...
$ python3 4_5.py
\end{lstlisting}
We observe that $\left|H\brak{e^{j\omega}}\right|$ is given by
\begin{align}
	\left|H\brak{e^{j\omega}}\right| &= \frac{1 + e^{-2j\omega}}{1 + \frac{1}{2}e^{-j\omega}}
\end{align}
and so its fundamental period is given by LCM($\frac{2\pi}{2}$, $2\pi$) = $2\pi$.
\begin{figure}[!ht]
	\centering
...
\solution The following code plots Fig. \eqref{fig:y-n-dft} and computes $X(k)$ and $Y(k)$. Note that this is the same as $y(n)$ in Fig. \eqref{fig:xnyn}.
...
\end{lstlisting}
To plot the impulse response function $h(n)$, we rewrite $H(z)$ using partial fractions to take the inverse $z$-transform.
\begin{align}
	H(z) = \sum_{i}\frac{r_i}{1 - p_iz^{-1}} + \sum_{j}k_jz^{-j}
\end{align}
...
\end{lstlisting}
We see that $h(n)$ is bounded and convergent. Also, since 1 is not a pole of the transfer function, the system is stable. The codes can be run all at once by typing
\begin{lstlisting}
$ python3 7_2_*.py
\end{lstlisting}
