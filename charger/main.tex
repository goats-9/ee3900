...
\solution The function $H(f)$ is a low pass filter which filters around the
...
given by
\begin{align}
    \abs{H\brak{f}}^2 = \frac{1}{\brak{1 + \brak{\frac{f}{f_c}}^2}^n}
\end{align}
...
\end{align}
Assuming the cutoff frequency to be at $f_0$, the Python code
...
\end{figure}
At $f = 10f_c$, we obtain a 40 dB loss for $n = 2$, which is
...
given by
\begin{align}
    \abs{H\brak{f}}^2 = \frac{1}{\brak{1 + \epsilon^2C_n^2\brak{\frac{f}{f_c}}^2}^n}
\end{align}
...
\end{figure}
At $f = 10f_c$, we obtain close to 40 dB loss for $n = 2$, which is
ideal. Hence, we choose $n = 2$. Further, note that the
DC gain is $\frac{5}{12}$.
